...
<title>Laporan Data Gejala – SiDeteksi</title>
<link rel="stylesheet" href="{{ asset('css/kop-surat.css') }}">
</head>
<body>
  <div class="no-print">
    <a href="{{ route('admin.laporan.index', ['tab'=>'gejala']) }}"
  ...
  <div class="page">
    ...
    <div class="laporan-judul">
      <div class="judul-text">Laporan Data Gejala</div>
    </div>

    {{-- Ringkasan --}}
    <div class="summary-grid" style="display:flex;justify-content:center">
      <div class="summary-box" style="width:220px">
        <div class="summary-value">{{ $gejala->count() }}</div>
        <div class="summary-label">Total Indikator Gejala</div>
      </div>
    </div>

    {{-- Tabel --}}
    <div class="section-title">Daftar Gejala</div>
    <table>
      <thead>
        <tr>
          <th style="width:30px">No</th>
          <th style="width:70px">Kode</th>
          <th>Nama / Deskripsi Gejala</th>
        </tr>
      </thead>
      <tbody>
        @forelse($gejala as $i => $g)
        <tr>
          <td style="text-align:center">{{ $i + 1 }}</td>
          <td><span class="badge-status" style="background:#4f46e5">{{ $g->kode }}</span></td>
          <td>{{ $g->nama }}</td>
        </tr>
        @empty
        <tr><td colspan="3" style="text-align:center;padding:1rem;color:#9ca3af">Tidak ada data gejala.</td></tr>
        @endforelse
      </tbody>
    </table>
